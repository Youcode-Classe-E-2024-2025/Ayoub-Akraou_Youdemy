...
class Course
{
   ...
   public function getAll($db, $search = '', $categoryId = null)
   {
      ...
      if (!empty($search)) {
         $sql .= " AND (MATCH(c.title, c.description) AGAINST(? IN BOOLEAN MODE) OR t.name LIKE ?)";
         $params[] = $search;
         $params[] = "%$search%";
      }
   ...
   public function getPopularCourses($db, $limit = 6)
   {
      $sql = "SELECT c.*, u.name as instructor_name, u.email as instructor_email, cat.name as category_name,
   ...
   public function getLatestCourses($db, $limit = 3)
   {
      $sql = "SELECT c.*, u.name as instructor_name, u.email as instructor_email, cat.name as category_name
              FROM courses c
              JOIN users u ON c.instructor_id = u.id
   ...
   public function searchCourses($db, $query)
   {
      $sql = "SELECT c.*, u.name as instructor_name, u.email as instructor_email, cat.name as category_name
              FROM courses c
              JOIN users u ON c.instructor_id = u.id
